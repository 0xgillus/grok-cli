    ...
    case 'skip':
      ...
      console.log(
        chalk.yellow('WARNING: ') +
          'You can set up your API key later with: ' +
          chalk.cyan('grok-cli config set-key')
      );
      console.log(
        chalk.gray('   Or set the environment variable: ') +
          chalk.cyan('export GROK_API_KEY="your-key"')
      );
      console.log();
      return false;
    default:
...
async function promptForApiKey(): Promise<boolean> {
  ...
  try {
    ...
    if (error.statusCode === 401) {
      console.log(
        chalk.yellow('   Invalid API key. Please check your key and try again.')
      );
