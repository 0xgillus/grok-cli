...
export class InteractiveChat {
  ...
  async start(): Promise<void> {
    ...
    if (!this.config.apiKey) {
      ...
      if (!setupComplete) {
        console.log(
          chalk.yellow('Setup skipped. You can configure your API key later.')
        );
        return;
      }
    ...
    try {
      ...
      if (this.userModels.length > 0) {
        ...
        if (this.config.defaultModel) {
          const configuredModel = this.userModels.find(
        ...
        const selectedModel = this.userModels.find(
          m => m.id === this.currentModel
        );
        this.maxContextLength = selectedModel?.contextLength || 32768;

        logger.info(
          `Using model: ${this.currentModel} (${this.userModels.length} available)`
        );
        logger.info(
          `Context length: ${this.maxContextLength.toLocaleString()} tokens`
        );
      } else {
  ...
  private async chatLoop(): Promise<void> {
    ...
    console.log(
      chalk.green('Ready to chat! Type ') +
        chalk.cyan('/help') +
        chalk.green(' for commands or ') +
        chalk.cyan('/quit') +
        chalk.green(' to exit.')
    );
    console.log();
  ...
  private showHelp(): void {
    ...
    console.log(
      chalk.gray('   /help     ') + chalk.white('- Show this help message')
    );
    console.log(
      chalk.gray('   /clear    ') + chalk.white('- Clear chat history')
    );
    console.log(
      chalk.gray('   /history  ') + chalk.white('- Show conversation history')
    );
    ...
    console.log(
      chalk.gray('   • ') + chalk.green('"Build me a Python CLI tool"')
    );
    console.log(
      chalk.gray('   • ') + chalk.green('"Make an Express API server"')
    );
    console.log(
      chalk.gray('   • ') + chalk.green('"Generate a Vue.js dashboard"')
    );
    console.log(
      chalk.gray('   • ') +
        chalk.green('"Create a mobile app with React Native"')
    );
    console.log(chalk.gray('   • ') + chalk.green('"Build a FastAPI backend"'));
    console.log(
      chalk.gray('   • ') + chalk.green('"Make a Next.js blog website"')
    );
    console.log();
    console.log(chalk.cyan('Tips:'));
    console.log(
      chalk.gray('   • ') +
        chalk.white('Ask questions about coding, debugging, or any topic')
    );
    console.log(
      chalk.gray('   • ') + chalk.white('Include code snippets for analysis')
    );
    console.log(
      chalk.gray('   • ') + chalk.white('Be specific for better responses')
    );
    console.log(
      chalk.gray('   • ') +
        chalk.white('Project creation works in any conversation')
    );
    console.log();
  }
  ...
  private async processUserMessage(input: string): Promise<void> {
    ...
    try {
      ...
      const percentageLeft = Math.max(
        0,
        (availableTokens / this.maxContextLength) * 100
      );
      ...
      if (percentageLeft > 75) {
        contextStatus = chalk.green(
          `${availableTokens.toLocaleString()} tokens available (${percentDisplay})`
        );
      } else if (percentageLeft > 25) {
        contextStatus = chalk.yellow(
          `${availableTokens.toLocaleString()} tokens available (${percentDisplay})`
        );
      } else {
        contextStatus = chalk.red(
          `${availableTokens.toLocaleString()} tokens available (${percentDisplay})`
        );
      }
      ...
      console.log(
        chalk.red('Error: ') +
          chalk.white(error.message || 'Failed to get response')
      );

      if (error.statusCode === 401) {
        console.log(
          chalk.yellow(
            '   Your API key may be invalid. Check your configuration.'
          )
        );
        console.log(
          chalk.gray('   Run: ') +
            chalk.cyan('grok-cli config set-key') +
            chalk.gray(' to update it.')
        );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('```')) {
        if (!inCodeBlock) {
          ...
          formattedLines.push(
            chalk.dim.gray('┌─ Code ') +
              chalk.cyan(codeLanguage || 'block') +
              chalk.dim.gray(' ─'.repeat(40 - (codeLanguage.length + 10)))
          );
        } else {
          // Ending code block
          inCodeBlock = false;
      ...
      if (inCodeBlock) {
        ...
        // Italic text (*text*)
        formattedLine = formattedLine.replace(/\*(.*?)\*/g, (match, p1) =>
          chalk.italic.gray(p1)
        );

        // Inline code (`code`)
        formattedLine = formattedLine.replace(/`([^`]+)`/g, (match, p1) =>
          chalk.bgBlackBright.hex('#a8e6cf')(' ' + p1 + ' ')
        );
  ...
  private isProjectCreationRequest(input: string): boolean {
    ...
    const hasDirectPattern = directPatterns.some(pattern =>
  ...
  private async handleProjectCreation(input: string): Promise<void> {
    ...
    try {
      ...
      if (projectDetails.framework) {
        console.log(
          chalk.gray('   Framework: ') + chalk.yellow(projectDetails.framework)
        );
      }
      console.log(
        chalk.gray('   Language: ') + chalk.magenta(projectDetails.language)
      );
      if (projectDetails.description) {
        console.log(
          chalk.gray('   Description: ') + chalk.dim(projectDetails.description)
        );
      }
      console.log();
      ...
      if (!confirm) {
        ...
        console.log(
          chalk.gray('You can always try again or use: ') +
            chalk.cyan('grok create')
        );
        return;
      }
      ...
      console.log(
        chalk.gray('   • ') +
          chalk.cyan('grok create') +
          chalk.gray(' for interactive creation')
      );
      console.log(
        chalk.gray('   • ') +
          chalk.cyan('grok create --help') +
          chalk.gray(' for all options')
      );
...
Return ONLY the JSON object, no other text:`;
    ...
    if (
      ...
      lowerInput.includes('cli') ||
      lowerInput.includes('command line') ||
      lowerInput.includes('terminal')
    ) {
      type = 'cli';
    } else if (
      lowerInput.includes('mobile') ||
      lowerInput.includes('react native')
    ) {
      type = 'mobile';
      language = 'typescript';
    }
