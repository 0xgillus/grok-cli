...
`;
  ...
  console.log(
    chalk.gray('                      ') +
      chalk.bold.hex('#FF6B6B')("Command-line interface for xAI's Grok models")
  );
  console.log();
  console.log(chalk.hex('#4ECDC4')('Tips for getting started:'));
  console.log(
    chalk.gray('1. ') +
      chalk.white('Ask questions, edit files, or run commands.')
  );
  console.log(
    chalk.gray('2. ') + chalk.white('Be specific for the best results.')
  );
  console.log(
    chalk.gray('3. ') +
      chalk.white('Create ') +
      chalk.cyan('GROK.md') +
      chalk.white(' files to customize your interactions with Grok.')
  );
  console.log(
    chalk.gray('4. ') +
      chalk.cyan('/help') +
...
export function displayFirstTimeSetup() {
  console.log(
    chalk.yellow('WARNING: ') + chalk.bold('First time setup required')
  );
  ...
  console.log(
    chalk.gray('   1. Visit ') + chalk.underline.blue('https://console.x.ai/')
  );
...
export async function displayContextInfo(
  ...
  try {
      ...
      if (models.length > 0) {
        ...
        if (config.defaultModel) {
          const configuredModel = models.find(
            (m: any) => m.id === config.defaultModel
          );
          if (configuredModel) {
        ...
        const selectedModel =
          models.find((m: any) => m.id === currentModel) || models[0];
    ...
    const percentageLeft = Math.max(
      0,
      (availableTokens / actualContextLength!) * 100
    );
    ...
    if (usedTokens && usedTokens > 0) {
      ...
      contextStatus = chalk.green(
        `${actualContextLength!.toLocaleString()} tokens available`
      );
    }
